...
class TestParseValidPDF:
    ...
    def test_parse_valid_pdf(self, mock_extract, client):
    ...
    @patch('routers.parse.extract_invoice')
    def test_parse_foreign_invoice_pdf(self, mock_extract, client):
...
class TestParseValidImage:
    ...
    @patch('routers.parse.extract_invoice')
    def test_parse_valid_image_jpg(self, mock_extract, client):
    ...
    @patch('routers.parse.extract_invoice')
    def test_parse_valid_image_png(self, mock_extract, client):
...
class TestParseFileValidation:
    ...
    def test_parse_rejects_invalid_extension(self, client):
    ...
    def test_parse_rejects_executable_files(self, client):
    ...
    def test_parse_rejects_no_filename(self, client):
        """Test rejection of files with no filename."""
        fake_file = BytesIO(b"fake content")
        ...
        # Should reject due to empty filename
        assert response.status_code in [415, 422]
    
    def test_parse_accepts_webp(self, client):
        """Test that WebP format is in allowed extensions."""
        from routers.parse import ALLOWED_EXTENSIONS
        assert "webp" in ALLOWED_EXTENSIONS
    
    def test_parse_rejects_mime_spoofing(self, client):
        """Test that MIME type spoofing is detected via magic bytes."""
        # A file claiming to be PDF but actually a PNG
        png_magic = b"\x89PNG\r\n\x1a\n" + b"\x00" * 100
        fake_file = BytesIO(png_magic)
        ...
        # Should reject because magic bytes don't match PDF
        assert response.status_code == 415
...
class TestParseConfidenceScore:
    ...
    @patch('routers.parse.extract_invoice')
    def test_parse_returns_confidence_score(self, mock_extract, client):
    ...
    @patch('routers.parse.extract_invoice')
    def test_parse_high_confidence(self, mock_extract, client):
    ...
    @patch('routers.parse.extract_invoice')
    def test_parse_medium_confidence(self, mock_extract, client):
    ...
    @patch('routers.parse.extract_invoice')
    def test_parse_low_quality_returns_low_confidence(self, mock_extract, client):
...
class TestParseErrorHandling:
    ...
    @patch('routers.parse.extract_invoice')
    def test_parse_extraction_failure(self, mock_extract, client):
    ...
    @patch('routers.parse.extract_invoice')
    def test_parse_unexpected_error(self, mock_extract, client):
    ...
    def test_parse_missing_file(self, client):
    ...
    def test_parse_empty_file(self, client):
        ...
        # Should fail — empty file or invalid PDF
        assert response.status_code in [415, 422, 413]
...
class TestParseProcessingTime:
    ...
    @patch('routers.parse.extract_invoice')
    def test_parse_returns_processing_time_header(self, mock_extract, client):
...
class TestParseDataIntegrity:
    ...
    @patch('routers.parse.extract_invoice')
    def test_parse_returns_complete_invoice_data(self, mock_extract, client):
